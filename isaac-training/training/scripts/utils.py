...
def evaluate(
    ...
    ).cpu()  # idx of first done will be return for each trajs
...
def vec_to_new_frame(vec, goal_direction):
    ...
    z_direction = torch.tensor([0, 0, 1.0], device=vec.device)
...
def vec_to_world(vec, goal_direction):
    world_dir = torch.tensor([1.0, 0, 0], device=vec.device).expand_as(goal_direction)
