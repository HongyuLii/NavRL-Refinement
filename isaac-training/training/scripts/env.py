...
from utils import construct_input, vec_to_new_frame
...
class NavigationEnv(IsaacEnv):
    ...
    def __init__(self, cfg):
        ...
        self.reward_config = {
            # "reward_vel": 1.0,
            # "reward_safety_static": 0.1,
            # "reward_safety_dynamic": 0.1,
            # "penalty_smooth": -0.5,
            "penalty_height": -0.1,
            "goal_bonus": 10.0,
            "distance_reward": 1.0,
        ...
        self.lidar_vfov = (
            max(-89.0, cfg.sensor.lidar_vfov[0]),
    ...
    def _design_scene(self):
        ...
        # Ground Plane
        cfg_ground = sim_utils.GroundPlaneCfg(color=(0.1, 0.1, 0.1), size=(300.0, 300.0))
        ...
        )  # in case of the roundup error

        # Dynamic obstacle info
        self.dyn_obs_list = []
        self.dyn_obs_state = torch.zeros(
            (self.cfg.env_dyn.num_obstacles, 13), dtype=torch.float, device=self.cfg.device
        )  # 13 is based on the states from sim, we only care the first three which is position
        self.dyn_obs_state[:, 3] = 1.0  # Quaternion
        ...
        )  # prefered distance between each dynamic obstacle
        ...
        for category_idx in range(cuboid_category_num + cylinder_category_num):
            ...
            for origin_idx in range(self.dyn_obs_num_of_each_category):
                ...
                while True:
                    ...
                    if category_idx < cuboid_category_num:
                        ...
                        oz = self.max_obs_2d_height / 2.0  # half of the height
                ...
                origin = [ox, oy, oz]
                self.dyn_obs_origin[
                    origin_idx + category_idx * self.dyn_obs_num_of_each_category
                ] = torch.tensor(origin, dtype=torch.float, device=self.cfg.device)
                self.dyn_obs_state[
                    origin_idx + category_idx * self.dyn_obs_num_of_each_category, :3
                ] = torch.tensor(origin, dtype=torch.float, device=self.cfg.device)
                prim_utils.create_prim(
                    f"/World/Origin{origin_idx + category_idx * self.dyn_obs_num_of_each_category}",
                    "Xform",
                    translation=origin,
                )
            ...
            if category_idx < cuboid_category_num:
                ...
                radius = (
                    float(category_idx - cuboid_category_num + 1) * max_obs_width / float(N_w) / 2.0
                )
                obs_width = radius * 2
            ...
            self.dyn_obs_size[
                category_idx * self.dyn_obs_num_of_each_category : (category_idx + 1)
                * self.dyn_obs_num_of_each_category
            ] = torch.tensor(
                [obs_width, obs_width, obs_height], dtype=torch.float, device=self.cfg.device
            )
    ...
    def move_dynamic_obstacle(self):
        ...
        dyn_obs_goal_dist = (
            torch.sqrt(torch.sum((self.dyn_obs_state[:, :3] - self.dyn_obs_goal) ** 2, dim=1))
            if self.dyn_obs_step_count != 0
            else torch.zeros(self.dyn_obs_state.size(0), device=self.cfg.device)
        )
        dyn_obs_new_goal_mask = (
            dyn_obs_goal_dist < 0.5
        )  # change to a new goal if less than the threshold

        # sample new goals in local range
        num_new_goal = torch.sum(dyn_obs_new_goal_mask)
        sample_x_local = -self.cfg.env_dyn.local_range[0] + 2.0 * self.cfg.env_dyn.local_range[
            0
        ] * torch.rand(num_new_goal, 1, dtype=torch.float, device=self.cfg.device)
        sample_y_local = -self.cfg.env_dyn.local_range[1] + 2.0 * self.cfg.env_dyn.local_range[
            1
        ] * torch.rand(num_new_goal, 1, dtype=torch.float, device=self.cfg.device)
        sample_z_local = -self.cfg.env_dyn.local_range[2] + 2.0 * self.cfg.env_dyn.local_range[
            2
        ] * torch.rand(num_new_goal, 1, dtype=torch.float, device=self.cfg.device)
        ...
        if self.dyn_obs_step_count % int(2.0 / self.cfg.sim.dt) == 0:
            ...
            self.dyn_obs_vel = (
                ...
                * (self.dyn_obs_goal - self.dyn_obs_state[:, :3])
                / torch.norm((self.dyn_obs_goal - self.dyn_obs_state[:, :3]), dim=1, keepdim=True)
            )
    ...
    def _set_specs(self):
        ...
        self.reward_spec = (
            CompositeSpec(
                {"agents": CompositeSpec({"reward": UnboundedContinuousTensorSpec((1,))})}
            )
            .expand(self.num_envs)
            .to(self.device)
        )
            ...
            CompositeSpec(
                {
                    ...
                    # auto-generate reward component fields
                    **{
                        key: UnboundedContinuousTensorSpec(1)
                        for key in self.reward_config.keys()
                    }
                }
    ...
    def reset_target(self, env_ids: torch.Tensor):
        if self.training:
            ...
            target_pos = 48.0 * torch.rand(
                env_ids.size(0), 1, 3, dtype=torch.float, device=self.device
            ) + (-24.0)
            heights = 0.5 + torch.rand(env_ids.size(0), dtype=torch.float, device=self.device) * (
                2.5 - 0.5
            )
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        if self.training:
            ...
            pos = torch.zeros(len(env_ids), 1, 3, device=self.device)
            pos[:, 0, 0] = (env_ids / self.num_envs - 0.5) * 32.0
            pos[:, 0, 1] = 24.0
            pos[:, 0, 2] = 2.0
        
        with torch.no_grad():
            dist0 = (self.target_pos[env_ids] - pos).norm(dim=-1, keepdim=True)
            self.prev_distance[env_ids] = dist0
    ...
    def _compute_state_and_obs(self):
        self.root_state = self.drone.get_state(
            env_frame=False
        )  # (world_pos, orientation (quat), world_vel_and_angular, heading, up, 4motorsthrust)
        ...
        rpos_clipped_g = vec_to_new_frame(
            rpos_clipped, target_dir_2d
        )  # express in the goal coodinate
        ...
        if self.cfg.env_dyn.num_obstacles != 0:
            ...
            dyn_obs_distance_2d = torch.norm(
                dyn_obs_rpos_expanded[..., :2], dim=2
            )  # Shape: (1000, 40). calculate 2d distance to each obstacle for all drones
            _, closest_dyn_obs_idx = torch.topk(
                dyn_obs_distance_2d,
                self.cfg.algo.feature_extractor.dyn_obs_num,
                dim=1,
                largest=False,
            )  # pick top N closest obstacle index
            dyn_obs_range_mask = (
            ...
            closest_dyn_obs_rpos = torch.gather(
                dyn_obs_rpos_expanded, 1, closest_dyn_obs_idx.unsqueeze(-1).expand(-1, -1, 3)
            )
            closest_dyn_obs_rpos_g = vec_to_new_frame(closest_dyn_obs_rpos, target_dir_2d)
            closest_dyn_obs_rpos_g[dyn_obs_range_mask] = 0.0  # exclude out of range obstacles
            ...
            dyn_obs_states = torch.cat(
                [
                    ...
                    closest_dyn_obs_width_category,
                    closest_dyn_obs_height_category,
                ],
                dim=-1,
            ).unsqueeze(1)
            ...
            )  # for those 2D obstacle, z distance will not be considered
        ...
        static_dist = (self.lidar_range - self.lidar_scan).clamp(
            min=1e-3, max=self.lidar_range
        )
        static_frac = static_dist / self.lidar_range
        reward_safety_static = torch.log1p(static_frac * (torch.e - 1.0)).mean(dim=(2, 3))

        # b. safety reward for dynamic obstacles
        if self.cfg.env_dyn.num_obstacles != 0:
            dyn_dist = closest_dyn_obs_distance_reward.clamp(
                min=1e-3, max=self.lidar_range
            )
            dyn_frac = dyn_dist / self.lidar_range
            reward_safety_dynamic = torch.log1p(
                dyn_frac * (torch.e - 1.0)
        ...
        # Distance reward, normalize by max dist
        distance_reward = -distance.squeeze(-1) / 40.0
        ...
        progress_reward = (self.prev_distance - current_distance).squeeze(-1).clamp(-0.5, 0.5)
        self.prev_distance = current_distance.detach().clone()

        # Calculate rewards
        reward_raw = {
            "reward_vel": reward_vel,
            "reward_safety_static": reward_safety_static,
            "reward_safety_dynamic": reward_safety_dynamic if self.cfg.env_dyn.num_obstacles != 0 else torch.zeros_like(reward_vel),
            "penalty_smooth": penalty_smooth,
            "penalty_height": penalty_height,
            "goal_bonus": reach_goal.float(), 
            "progress_reward": progress_reward,
            "distance_reward": distance_reward,
        }
        reward_registry = {
            key: reward_raw[key] * weight
            for key, weight in self.reward_config.items()
        }
        self.reward = sum(reward_registry.values())

        # terminated penalty
        self.reward[self.terminated] -= 10.0
        ...
        # auto-log reward components
        for key, val in reward_registry.items():
            self.stats[key] = val.detach()
