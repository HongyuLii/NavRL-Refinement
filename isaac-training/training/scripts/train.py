...
def main(cfg):
    ...
    # Algorithm (selected via cfg.algo.name)
    policy = make_algo(
        cfg.algo,
    ...
    if cfg.checkpoint.load:
        if not cfg.checkpoint.path:
            raise ValueError("checkpoint.load is True but checkpoint.path is empty.")
        state_dict = torch.load(cfg.checkpoint.path, map_location=cfg.device)
        policy.load_state_dict(state_dict, strict=True)
        print(f"[NavRL]: loaded checkpoint from {cfg.checkpoint.path}")
    ...
    for i, data in enumerate(collector):
        ...
        if len(episode_stats) >= transformed_env.num_envs:
            stats = {
                "train/" + (".".join(k) if isinstance(k, tuple) else k): torch.mean(
                    v.float()
                ).item()
